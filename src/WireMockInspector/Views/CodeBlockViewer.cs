...
using WireMockInspector.Views.Transformer;
...
namespace WireMockInspector.Views
{
    public class CodeBlockViewer : TextEditor
    {
        ...
        {
            (e.Sender as CodeBlockViewer)?.OnCodeChanged((ViewModels.MarkdownCode)e.OldValue,
                (ViewModels.MarkdownCode)e.NewValue);
        }
        ...
        private void SetMarkdown(ViewModels.MarkdownCode md)
        {
            if (md is not null)
            {
                ...
                if (_currentLang != md.lang)
                {
                    ...
                    if (this.TextArea.TextView.LineTransformers.OfType<DiffLineBackgroundRenderer>().FirstOrDefault() is
                        { } existing)
                    {
        ...
        public static readonly StyledProperty<ViewModels.MarkdownCode> CodeProperty =
            AvaloniaProperty.Register<CodeBlockViewer, ViewModels.MarkdownCode>(nameof(Code));
